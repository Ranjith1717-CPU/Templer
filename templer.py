...
- **Dynamic Values Found:** {len(analysis.dynamic_values)}
- **LLM Sections:** {analysis.dynamic_llm_count}
- **Static Sections:** {analysis.static_count}
...
"""
    ...
    for dv in analysis.dynamic_values:
        report += f"| `{{{{{dv.placeholder_name}}}}}` | {dv.value_type} | {dv.original_text[:30]} | {dv.context[-30:] if dv.context else '-'} |\n"
    ...
    if llm_sections:
        ...
        for section in llm_sections:
            report += f"### `{{{{LLM:{section.placeholder_name or 'unnamed'}}}}}`\n"
            ...
            report += f"- **Sample:** {section.text[:100]}...\n\n"
    ...
    if mode == "🔧 Analyze & Create Template":
        ...
        **This turns a 4-hour manual process into minutes of review.**
        ...
        if st.session_state.analysis_result:
            ...
            if analysis.dynamic_values:
                with st.expander(f"🎯 Dynamic Values Found ({len(analysis.dynamic_values)} placeholders)", expanded=True):
                    ...
                    by_type = {}
                    for dv in analysis.dynamic_values:
                        if dv.value_type not in by_type:
                            by_type[dv.value_type] = []
                        by_type[dv.value_type].append(dv)
                    ...
                    for value_type, values in by_type.items():
                        ...
                        for dv in values:
                            ...
                            with col1:
                                st.code(f"{{{{{dv.placeholder_name}}}}}")
                            with col2:
                                st.text(f"Original: \"{dv.original_text}\"")
                                if dv.context:
                                    st.caption(f"Context: ...{dv.context[-40:]}...")
                        st.markdown("---")
            else:
